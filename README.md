...
The **Gulf Coast** is a critical hub for the United States' energy sector, facing increasing threats from climate change (e.g., severe storms, sea-level rise) and active geological processes (e.g., land subsidence). Maintaining the stability and resilience of its vast transmission line infrastructure is **parmount** for national energy security and the region's economy.

This project utilizes cutting-edge InSAR (Interferometric Synthetic Apertar Radar) data from the NASA/JPL **OPERA** mission to systematically assess and classify the deformation risk of transmission towers across the Gulf Coast.
...
| **2. Data Pre-Processing** | `Opera-Vertical-Mask-Reproject-Processcer.py` | Converts LOS displacement to **vertical displacement** using incidence angles, applies masking, and reprojects data into dual-band GeoTIFFs (Vertical Displacement + Coherence). | Dual-band GeoTIFFs |
